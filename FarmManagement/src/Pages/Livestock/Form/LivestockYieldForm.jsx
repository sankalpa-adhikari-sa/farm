...
import { useAddYieldData } from '@/hooks/useYieldData';
function LivestockYieldForm(props) {
  const form= useForm({
    defaultValues: {
      yield_details:"",
      yield_unit:"",
  ...
  const is_yield_loss = useWatch({ control: form.control, name: 'is_yield_loss', defaultValue: 'no' });
  const {id}= useParams()
  const {data:type_query}= useLivestockByID(id)
  const type_data=type_query?.expand.livestock_type.livestock_type_yield
  const YIELD_TYPE= type_data?type_data.map((item => ({
      ...
      <FormField
        ...
        render={({field}) =>(
          <FormItem >
            ...
            Yield loss quantity will be showed as option while adding yield
            </FormDescription>
