...
function IndvLivestock() {
  ...
  const DropdownMenuTriggerCSS =
    "text-secondary-foreground border border-input h-10 px-4 py-2 bg-background inline-flex items-center justify-center hover:bg-accent hover:text-accent-foreground border-dashed border-2";
  return (
          ...
          <div>
            <DropdownMenu>
              ...
              <DropdownMenuContent className=" w-40 mr-2">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  onClick={() => navigate(`/livestock/${id}/resource_usage`)}
                  className="h-10"
                >
                  Resource Usage
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => navigate(`/livestock/${id}/yield`)}
                  className="h-10"
                >
                  Yield
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => navigate(`/livestock/${id}/measurement`)}
                  className="h-10"
                >
                  Measurement
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => navigate(`/livestock/${id}/treatment`)}
                  className="h-10"
                >
                  Treatment
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => navigate(`/livestock/${id}/details`)}
                  className="h-10"
                >
                  Details
                </DropdownMenuItem>
